...
class TawaqueatSupabaseUser extends BaseAuthUser {
  TawaqueatSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> tawaqueatSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = TawaqueatSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
